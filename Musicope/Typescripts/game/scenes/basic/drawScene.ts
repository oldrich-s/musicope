module Musicope.Game.Scenes.BasicFns {
  ...
  export interface Input {
    ...
    readOnly: Params.IParamsData;
    pixelsPerTime: number;
    sceneWidth: number;
    sceneHeight: number;
    tracks: Songs.INoteScene[][];
    sustainNotes: Songs.ISustainNoteScene[];
    p_minNote: number;
    p_maxNote: number;
  ...
  function getColorByVelocity(color: number[], velocity: number, minMaxVel: number[]) {
    ...
    if (Math.abs(minMaxVel[1] - minMaxVel[0]) > 10) {
      var scale = 0.35 + 0.65 * (velocity - minMaxVel[0]) / (minMaxVel[1] - minMaxVel[0]);
